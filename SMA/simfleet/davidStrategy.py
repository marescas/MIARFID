"""
En esta estrategia el cliente recibe  las posiciones de todos los transportes y los organiza en base a la posición
De esta forma el cliente escogera el transporte que este más cerca
    ...
    async def run(self):
        ...
        if protocol == QUERY_PROTOCOL:
            ...
            if performative == REQUEST_PERFORMATIVE:
                ...
                #Ahora como cada cliente tiene una lista de transportes puede ser que uno de estos
                #reciba un rechazo lo que implica un cambio de estado pero... y si esta con un cliente?
                #Debemos comprobar que solo transitara de estado en el caso de que este libre
...
class MyCustomerStrategy(CustomerStrategyBehaviour):
    ...
    async def on_start(self):
        ...
        """
        self.agent.set("yaBastaTronco", False)
        ...
        """
        t = threading.Thread(target=lambda : threading.Timer(4, lambda : self.agent.set("yaBastaTronco", True)).start())
        t.start()
    ...
    async def run(self):
        ...
        if msg:
            ...
            if performative == PROPOSE_PERFORMATIVE:
                if self.agent.status == CUSTOMER_WAITING:  
                    ...
                    #distance=  distance_in_meters(self.agent.get_position(), transport_position)
        ...
        if self.agent.status == CUSTOMER_WAITING:
            if self.agent.get("yaBastaTronco") == True and len(self.agent.get("misOportunidades")) > 0:
